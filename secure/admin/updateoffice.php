<?php
	...
	$officeupdate = mysqli_prepare($conn, "UPDATE 7062protutordetails SET RoomNumber=?, RoomAddress=?, PhoneExtension=? WHERE User_ID=?");
	mysqli_stmt_bind_param($officeupdate, 'sssi', $roomnumber, $roomaddress, $phoneext, $id);
	
	mysqli_stmt_execute($officeupdate);
	
	mysqli_stmt_close($officeupdate);
